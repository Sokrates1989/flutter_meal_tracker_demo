...
class DataHandlerMealRepo {
  ...
  Future<List<String>?> getMealTypes() async {
    ApiReturn apiResponse = await _apiConnector.getMealRepo().getMealTypes();
    ...
    List<String> mealTypes = List<String>.from(apiResponse.data.map((meal) => meal['name']));
    return mealTypes;
  }
  ...
  Future<bool> addMeal(Meal meal) async {
    ApiReturn apiResponse = await _apiConnector.getMealRepo().addMeal(meal);
